...
def batchnorm_forward(x, gamma, beta, bn_param):
	...
	cache = xhat, eps+sample_var,gamma
...
def batchnorm_backward(dout, cache):
    ...
    xhat,sample_var,gamma = cache
    N, D = dout.shape
    
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(xhat*dout,axis=0)
    dxhat = dout*gamma[np.newaxis,:]
    dvar = -0.5*np.sum(dxhat*xhat/sample_var[np.newaxis,:],axis=0)
    dmu = np.sum(-dxhat/np.sqrt(sample_var[np.newaxis,:]),axis=0)
    dmu += -2*dvar*np.sum(xhat*np.sqrt(sample_var[np.newaxis,:]),axis=0)/N
    dx = dxhat/np.sqrt(sample_var[np.newaxis,:])
    dx += 2*dvar*xhat*np.sqrt(sample_var[np.newaxis,:])/N
    dx += dmu[np.newaxis,:]/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    #dx = dout*gamma/sample_sd + (-np.sum(dout*gamma,axis=0)-xhat*np.sum(dout*gamma*xhat,axis=0))/(N*sample_sd)
    dx = gamma*(dout - (dbeta+xhat*dgamma)/N)/sample_sd
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    xnew = np.pad(x.copy(),pad_dim,'constant') # Use default zero-padding
    ...
    for i in range(N):
        for j in range(F):
	    for kout,k in enumerate(range(0,H + 2 * pad - HH + 1 ,stride)):
	    	for lout,l in enumerate(range(0,W + 2 * pad - WW + 1,stride)):
			out[i,j,kout,lout] = np.sum(xnew[i,:,k:(k+HH),l:(l+WW)]*w[j,:,:,:]) + b[j]
...
def conv_backward_naive(dout, cache):
    ...
    xnew = np.pad(x.copy(),pad_dim,'constant') # Use default zero-padding
    dxnew = np.zeros_like(xnew)
    
    for i in range(N):
        for j in range(F):
	    for kout,k in enumerate(range(0,H + 2 * pad - HH + 1 ,stride)):
	    	for lout,l in enumerate(range(0,W + 2 * pad - WW + 1,stride)):
			dw[j,:,:,:] += (dout[i,j,kout,lout]*xnew[i,:,k:(k+HH),l:(l+WW)])
			dxnew[i,:,k:(k+HH),l:(l+WW)] += (dout[i,j,kout,lout]*w[j,:,:,:])

    dx = dxnew[:,:,pad:(H+pad),pad:(W+pad)]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    Wnew = 1 + (W - pool_height) / stride

    out = np.zeros((N,C,Hnew,Wnew))

    for i in range(N):
        for j in range(C):
	    for kout,k in enumerate(range(0,H - pool_height + 1 ,stride)):
	    	for lout,l in enumerate(range(0,W - pool_width + 1,stride)):
			out[i,j,kout,lout] = np.max(x[i,j,k:(k+pool_height),l:(l+pool_width)]) 
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
	    ...
	    for kout,k in enumerate(range(0,H - pool_height + 1 ,stride)):
	    	for lout,l in enumerate(range(0,W - pool_width + 1,stride)):
			#out[i,j,kout,lout] = np.max(x[i,j,k:(k+pool_height),l:(l+pool_width)]) 
			maxval = np.max(x[i,j,k:(k+pool_height),l:(l+pool_width)])
			mask = ( x[i,j,k:(k+pool_height),l:(l+pool_width)] == maxval )
			dx[i,j,k:(k+pool_height),l:(l+pool_width)] += (dout[i,j,kout,lout]*mask)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    cache = [] 
    for i in range(C):
	out1_temp,out1_cache = batchnorm_forward(np.reshape(x[:,i,:,:],(N,H*W)),gamma[i],beta[i],bn_param)
    	out[:,i,:,:] = np.reshape(out1_temp,(N,H,W))
	cache.append(out1_cache)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dbeta = np.zeros(C)
    for i in range(C):
	dx_i,dgamma_i,dbeta_i = batchnorm_backward_alt(np.reshape(dout[:,i,:,:],(N,H*W)),cache[i])
    	dx[:,i,:,:] = np.reshape(dx_i,(N,H,W))
